...
public class IHaveTheTruthWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "I Have The Truth";
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("span.author a").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div.social-sharing-top,"
        + "div#author-text,"
        + "div#sidebar-wrapper"))
    {
      element.remove();
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("https://ihavethetruth.com/");
  }
}
