...
public class PoliticoEU extends Wrapper {
	...
	/* Call previous parse function and add URI and
	 * veracity to the generated article object */
	public Article parse(final Document jsoupDocument,
		      final String uri, final Veracity veracity) {
		Article article = parse(jsoupDocument);
		article.setUri(uri);
		article.setVeracity(veracity);
		return article;		
	}

	/* Parse with jsoup, then parse with previous function and
	 * add URI and veracity to the article object */
	public Article parse(final String html,
		      final String uri, final Veracity veracity) {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("footer.meta p span.byline span.vcard a.url.fn").text();
	}


	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("header h1").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("p.subhead,"
				+ "p.wp-caption-text,"
				+ "aside"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.politico.eu/");
	}
}
