...
public class DepartedWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.post-meta-author a").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("p.post-meta,"
				+ "p.commentcount,"
				+ "div.afterwp,"
				+ "section#check-also-box,"
				+ "div.ulp-content"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://departed.co/");
	}
}
