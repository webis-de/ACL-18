...
public class TheNewsNerdWrapper extends Wrapper {
	@Override
	public Article parse(Document jsoupDocument) {
		Elements tags = jsoupDocument.select("div[id=\"content-area\"]");
		/* Extract Title, Main text, Paragraph Structure,
		 * Quotes, Links */
		String title = getTitle(jsoupDocument);
		String mainText = getMainText(tags);
		String author = getAuthor(jsoupDocument);
		String portal = "The News Nerd";
		String orientation = "fake news";
		List<Span> paragraphs = getParagraphs(tags);
		List<Span> links = getLinks(tags);
		List<Span> quotes = getQuotes(tags);
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.post-byline a").text();
	}
	...
	private String getMainText(Elements tags) {
		return tags.select("p").text();
	}

	private List<Span> getParagraphs(Elements tags) {
		List<Span> paragraphs = new ArrayList<Span>(); 
		Elements para = tags.select("p");
	...
	private List<Span> getLinks(Elements tags) {
		Elements paragraphs = tags.select("p");
	...
	private List<Span> getQuotes(Elements tags) {
		String mainText = tags.select("p").text();
		Elements quotes = tags.select("blockquote");
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.thenewsnerd.com/");
	}
}
