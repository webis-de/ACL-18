...
public class FreedomDailyWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {    
    return jsoupDocument.select("div.td-post-author-name a").first().text();
  }
  ...
  /* Filter unnessecary content */
  public Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div.essb_mailform_content,"
        + "blockquote[cite='https://www.facebook.com/FreedomDailyNews/']"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://freedomdaily.com/");
  }
}
