...
public class PolitistickWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.entry-meta-author.vcard.author a.fn").text();
	}

	
	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("h2").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("p.entry-meta,"
				+ "div.textwidget widget-text,"
				+ "div.footer-widget.mh_newsdesk_authors,"
				+ "div.footer-bottom,"
				+ "div.essb_mailform,"
				+ "div.signup-fields"))
		{
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://politistick.com/");
	}	
}
