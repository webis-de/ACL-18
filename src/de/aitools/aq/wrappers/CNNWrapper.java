...
public class CNNWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.metadata__byline__author").text();
	}
	...
	private String getMainText(Document jsoupDocument) {
		...
		return jsoupDocument.select("p.zn-body__paragraph,"
				+ "div.zn-body__paragraph").text();
	}
	...
	private List<Span> getParagraphs(Document jsoupDocument) {
		...
		Elements para = jsoupDocument.select("p.zn-body__paragraph,"
	...
	private List<Span> getLinks(Document jsoupDocument) {
		Elements paragraphs = jsoupDocument.select("p.zn-body__paragraph,"
				+ "div.zn-body__paragraph");
	...
	private List<Span> getQuotes(Document jsoupDocument) {
		String mainText = jsoupDocument.select("p.zn-body__paragraph,"
				+ "div.zn-body__paragraph").text();
	...
	public boolean isValidUri(String targetUri) {
		if(targetUri.contains("http://www.cnn.com/") ||
				targetUri.contains("http://edition.cnn.com/"))
			return true;
		else
			return false;
	}
