...
public class EagleRisingWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.fn a").text();
	}
	...
	private List<Span> getLinks(Document jsoupDocument) {
		filter(jsoupDocument);
		Elements paragraphs = jsoupDocument.select("h5," + "p");
	...
	private List<Span> getQuotes(Document jsoupDocument) {
		filter(jsoupDocument);
		String mainText = jsoupDocument.select("h5," + "p").text();
		Elements quotes = jsoupDocument.select("blockquote");
		List<Span> quoteObjects = new ArrayList<Span>();
		
		/* Filter ads */
		for(Element element : quotes.select("div[class=\"pan-ad-inline2\"]"))
		{
			element.remove();
		}
	...
	public Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div#widget-form,"
				+ "footer,"
				+ "p[style='text-align: center; margin: 0px 0px 6px 3px;'],"
				+ "div.essb_mailform,"
				+ "div.data-content"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://eaglerising.com/");
	}
}
