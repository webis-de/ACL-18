...
public class GristWrapper extends Wrapper {
	@Override
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("a.author.url.fn").text();
	}

	private String getTitle(Element tags) {
		return tags.select("h1").text();
	}
	
	private String getMainText(Element tags) {
		return tags.select("p").text();
	}

	private List<Span> getParagraphs(Element tags) {
	...
	private List<Span> getLinks(Element tags) {
	...
	private List<Span> getQuotes(Element tags) {
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://grist.org/");
	}
}
