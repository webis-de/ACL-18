...
public class WorldNewsDailyReportWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "World News Daily Report";
  ...
  private String getAuthor(Document jsoupDocument) {
    return "World News Daily Report";
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div.comments-wrap,"
        + "div#respond,"
        + "footer"))
    {
      element.remove();
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://worldnewsdailyreport.com/");
  }
}
