...
public class ConstitutionWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("span.wpdev-meta-item.wpdev-meta-author a").text();
  }

  private String getTitle(Document jsoupDocument) {
    return jsoupDocument.select("div.wpdev-entry-background h1").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div.textwidget.widget-text,"
        + "div.essb_mailform,"
        + "div.col-sm-10"))
    {
      element.remove();
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://constitution.com/");
  }
}
