...
public class ReelNewsNetworkWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "Reel News Network";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.vcard.author span.fn a").first().text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div.main-box-inside,"
				+ "div#comments,"
				+ "div.container-full site-info"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.reelnewsnetwork.com/");
	}
}
