...
public class AmericanMilitaryNewsWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "American Military News";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.vcard.author span.fn a").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("p.post-byline,"
				+ "p.post-tags,"
				+ "footer,"
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://americanmilitarynews.com/");
	}
}
