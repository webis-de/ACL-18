...
public class USHeraldWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("p.postmetadata a").text();
	}


	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("h1.title").text();
	}
	...
	private List<Span> getQuotes(Document jsoupDocument) {
		...
		/* Get substrings between quotation marks 2*/
		Pattern pattern3 = Pattern.compile("\"(.*?)\"");
		Matcher matcher3 = pattern3.matcher(mainText);
		
		while(matcher3.find())
		{
			String word = matcher3.group(1);
			int start = mainText.indexOf(word);
			int end = start + word.length();
	...
	private Document filter(Document jsoupDocument){
		for(Element element : jsoupDocument.select("p.postmetadata,"
				+ "div.bodyText,"
				+ "div#footer,"
				+ "div.essb_mailform"))
		{
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://usherald.com/");
	}
}
