...
public class HuffingtonPostWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.author-card__details__name").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("section.follow-us__window"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.huffingtonpost.com/");
	}
}
