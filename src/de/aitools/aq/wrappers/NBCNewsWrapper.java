...
public class NBCNewsWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("span.byline_author").text();
  }
  ...
  private String getMainText(Document jsoupDocument) {
    return jsoupDocument.select("div.nbcnews-dc-section,"
        + "div.article-body").first().select("p").text();
  }

  private List<Span> getParagraphs(Document jsoupDocument) {
    List<Span> paragraphs = new ArrayList<Span>(); 
    Elements para = jsoupDocument.select("div.nbcnews-dc-section,"
        + "div.article-body").first().select("p");
  ...
  private List<Span> getLinks(Document jsoupDocument) {
    Elements paragraphs = jsoupDocument.select("div.nbcnews-dc-section,"
        + "div.article-body").first().select("p");
  ...
  private List<Span> getQuotes(Document jsoupDocument) {
    String mainText = jsoupDocument.select("div.nbcnews-dc-section,"
        + "div.article-body").first().select("p").text();
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.nbcnews.com/");
  }
}
