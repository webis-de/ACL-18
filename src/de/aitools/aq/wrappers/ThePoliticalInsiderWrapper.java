...
public class ThePoliticalInsiderWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "The Political Insider";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("div.byline-author a").first().text();
	}


	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("header div h2").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("p.ie8only,"
				+ "div.textwidget,"
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.thepoliticalinsider.com/");
	}
}
