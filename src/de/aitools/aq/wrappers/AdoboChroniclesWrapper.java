...
public class AdoboChroniclesWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "Adobo Chronicles";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("a.url.fn.n").first().text();
	}
	...
	private String getMainText(Document jsoupDocument) {
		return jsoupDocument.select("div.entry-content p").text();
	}

	private List<Span> getParagraphs(Document jsoupDocument) {
		List<Span> paragraphs = new ArrayList<Span>(); 
		Elements para = jsoupDocument.select("div.entry-content p");
	...
	private List<Span> getLinks(Document jsoupDocument) {
		Elements paragraphs = jsoupDocument.select("div.entry-content p");
	...
	private List<Span> getQuotes(Document jsoupDocument) {
		String mainText = jsoupDocument.select("div.entry-content p").text();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("https://adobochronicles.com/");
	}
}
