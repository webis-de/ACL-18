...
public class USUncutWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.author-names a").text();
	}
	...
	public Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div.archive-list-text.left.relative,"
				+ "div.textwidget,"
				+ "em"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://usuncut.com/");
	}
}
