...
public class FoxNewsWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("div p span a").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div.branding,"
        + "div.article-info,"
        + "div.caption,"
        + "footer,"
        + "div.newsletter"))
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.foxnews.com/");
  }
}
