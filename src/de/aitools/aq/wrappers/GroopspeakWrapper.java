...
public class GroopspeakWrapper extends Wrapper {

	/* This class works for both author.groopspeak.com as well
	 * as news.groopspeak.com */
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("a.url.fn.n").text();
	}

	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("div.home-featuredtitle.single-featuredtitle").text();
	}
	...
	public boolean isValidUri(String targetUri) {
		if(targetUri.contains("http://news.groopspeak.com/")
				|| targetUri.contains("http://author.groopspeak.com/"))
			return true;
		else
			return false;
	}
