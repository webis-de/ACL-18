...
public class VoxWrapper extends Wrapper {
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("p.m-entry__byline,"
        + "div.l-full.footer,"
        + "div.c-article-feedback,"
        + "div.c-entry-box__body.has-meta,"
        + "div.c-river__entry"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.vox.com/");
  }
}
