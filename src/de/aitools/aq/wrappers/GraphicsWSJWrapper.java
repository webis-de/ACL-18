...
public class GraphicsWSJWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "Wallstreet Journal (Graphics)";
  ...
  private List<Span> getLinks(Document jsoupDocument) {
    Elements paragraphs = jsoupDocument.select("p.splitter");
  ...
  private List<Span> getQuotes(Document jsoupDocument) {
    String mainText = jsoupDocument.select("p.splitter").text();
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://graphics.wsj.com/");
  }
}
