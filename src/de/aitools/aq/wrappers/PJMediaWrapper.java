...
public class PJMediaWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("div.author a").text();
	}


	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("h3").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div.print-only"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("https://pjmedia.com/");
	}
}
