...
public class LGBTNationWrapper extends Wrapper {
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("p.wp-caption-text"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.lgbtqnation.com/");
  }
}
