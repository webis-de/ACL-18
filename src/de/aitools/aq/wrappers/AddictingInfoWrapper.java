...
public class AddictingInfoWrapper extends Wrapper {
  ...
  public String getMainText(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("em"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    if(targetUri.contains("http://www.addictinginfo.org") 
        || targetUri.contains("http://author.addictinginfo.org")
        || targetUri.contains("http://addictinginfo.org"))
      return true;
    else
      return false;
  }
