...
public class AmericanNewsWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("span.author.vcard a.url.fn.n b").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("p.simple-share,"
        + "div.related-posts.clearfix"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://americannews.com/");
  }
}
