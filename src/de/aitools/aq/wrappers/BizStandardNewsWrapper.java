...
public class BizStandardNewsWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "Biz Standard News";
  ...
  private String getAuthor(Document jsoupDocument) {
    Elements div = jsoupDocument.select("div.entry-content");
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("h1.site-title,"
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://bizstandardnews.com/");
  }
}
