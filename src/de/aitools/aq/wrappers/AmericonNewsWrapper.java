...
public class AmericonNewsWrapper extends Wrapper {
  @Override
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("a.fn").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("p.mh-meta.entry-meta,"
        + "div.mh-container.mh-container-inner.clearfix,"
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://americonnews.com/");
  }
}
