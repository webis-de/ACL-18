...
public class NYTimesWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("span.byline-author").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div#TopAd,"
        + "p.byline-column.has-single-author,"
        + "footer,"
        + "div.reader-satisfaction-survey.prompt.feedback-prompt.story-content.hidden,"
        + "section#site-index"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.nytimes.com/");
  }
}
