...
public class Superstation95Wrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.itemAuthor a").text();
	}

	
	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("h3.itemTitle").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("form,"
				+ "div.module.clearfix,"
				+ "div.itemComments"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("https://www.superstation95.com/");
	}
}
