...
public class PoliticoWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("footer.meta p.byline span.vcard").text();
	}


	private String getTitle(Document jsoupDocument) {
		/* Remove global heading ("POLITICO") */
		for(Element element : jsoupDocument.select("div.global-branding"))
		{
			element.remove();
		}
	...
	public Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("footer,"
				+ "div.summary,"
				+ "header.timeline-end-header,"
				+ "figcaption,"
				+ "aside,"
				+ "div.footer-group.footer-legal"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.politico.com/");
	}
}
