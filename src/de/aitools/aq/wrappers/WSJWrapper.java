...
public class WSJWrapper extends Wrapper {
  ...
  public Article parse(Document jsoupDocument) {
    ...
    String portal = "Wall Street Journal";
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("div.author.hasMenu span").text();
  }
  ...
  private List<Span> getLinks(Document jsoupDocument) {
    Elements paragraphs = jsoupDocument.select("article p");
  ...
  private List<Span> getQuotes(Document jsoupDocument) {
    String mainText = jsoupDocument.select("article p").text();
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.wsj.com/");
  }
}
