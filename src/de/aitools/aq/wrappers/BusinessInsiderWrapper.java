...
public class BusinessInsiderWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "Business Insider";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("li.single-author a").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div[style='display:none'],"
				+ "span.source,"
				+ "ul.media-list"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.businessinsider.com/");
	}
}
