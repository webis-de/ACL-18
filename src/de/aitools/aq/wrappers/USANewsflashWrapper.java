...
public class USANewsflashWrapper extends Wrapper {
  @Override
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div#comments"))
    {
      element.remove();
    }
    jsoupDocument.select("p").last().remove();
    return jsoupDocument;
  }
  @Override
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://usanewsflash.com/");
  }
}
