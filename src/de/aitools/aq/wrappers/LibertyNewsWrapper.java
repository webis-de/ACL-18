...
public class LibertyNewsWrapper extends Wrapper{
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("div.post-author a").text();
	}

	
	private String getTitle(Document jsoupDocument) {
		return jsoupDocument.select("h1[itemprop='headline']").text();
	}
	
	private String getMainText(Document jsoupDocument) {
		return jsoupDocument.select("div[itemprop='articleBody'] p").text();
	}

	private List<Span> getParagraphs(Document jsoupDocument) {
		List<Span> paragraphs = new ArrayList<Span>(); 
		Elements para = jsoupDocument.select("div[itemprop='articleBody'] p");
	...
	private List<Span> getLinks(Document jsoupDocument) {
		Elements paragraphs = jsoupDocument.select("div[itemprop='articleBody'] p");
	...
	private List<Span> getQuotes(Document jsoupDocument) {
		String mainText = jsoupDocument.select("div[itemprop='articleBody'] p").text();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://libertynews.com/");
	}
}
