...
public class FoxNewsInsiderWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "Fox News (Insider)";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span#author a span").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("strong,"
				+ "div.newsletter-form.under-article,"
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://insider.foxnews.com/");
	}
}
