...
public class BreitbartWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("a.byauthor").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("p.byline,"
        + "p.sh2,"
        + "footer,"
        + "div#comments,"
        + "div.signup,"
        + "div.send-tip.SWW,"
        + "div.mobile.send-tip.mobile-send-tip.SWW,"
        + "p.head"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.breitbart.com/");
  }
}
