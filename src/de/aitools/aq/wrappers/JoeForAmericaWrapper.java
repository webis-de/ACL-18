...
public class JoeForAmericaWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.reviewer a").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div.essb_mailform,"
				+ "div.description,"
				+ "div.inline-signup"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://joeforamerica.com/");
	}
}
