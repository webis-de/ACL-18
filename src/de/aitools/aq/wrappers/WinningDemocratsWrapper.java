...
public class WinningDemocratsWrapper extends Wrapper {
	...
	public Article parse(Document jsoupDocument) {
		...
		String portal = "Winning Democrats";
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("div.post_detail").text();
	}


	private String getTitle(Document jsoupDocument) {
		/* Get title only, remove unnessecary Strings */
		for(Element element : jsoupDocument.select("h2.widgettitle"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://winningdemocrats.com/");
	}
}
