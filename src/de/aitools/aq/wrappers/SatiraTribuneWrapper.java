...
public class SatiraTribuneWrapper extends Wrapper {
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div#disqus_thread"))
    {
      element.remove();
    }
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://www.satiratribune.com/");
  }
}
