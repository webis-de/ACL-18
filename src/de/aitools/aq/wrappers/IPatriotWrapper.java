...
public class IPatriotWrapper extends Wrapper {
  ...
  private String getAuthor(Document jsoupDocument) {
    return jsoupDocument.select("span.author-name").text();
  }
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("div.modal-body,"
        + "div.col-sm-10,"
  ...
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://ipatriot.com/");
  }
}
