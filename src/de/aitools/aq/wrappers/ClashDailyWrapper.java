...
public class ClashDailyWrapper extends Wrapper {
  ...
  private Document filter(Document jsoupDocument) {
    for(Element element : jsoupDocument.select("p[style='display: inline-block; margin:0;'],"
        + "div.essb_mailform,"
        + "div.textwidget.widget-text"))
    {
      element.remove();
    }
    jsoupDocument.select("p").last().remove(); // Ad Blocker message
    return jsoupDocument;
  }
  @Override
  public boolean isValidUri(String targetUri) {
    return targetUri.contains("http://clashdaily.com/");
  }
}
