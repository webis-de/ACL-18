...
public class LiberalAmericaWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("p.single_postmeta span a").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("p.single_postmeta,"
				+ "footer,"
				+ "div.widgetinner"))
		{
			element.remove();
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://www.liberalamerica.org/");
	}
}
