...
public class WashingtonPostWrapper extends Wrapper {
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("span.pb-byline span").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div.notifications-controll,"
				+ "div.permission-request.hide,"
				+ "div.shareBar-follow-modal,"
				+ "div.subTitle-follow-modal,"
				+ "div#signup-box-rr,"
				+ "div.label-wrapper.text-align-inherit.games-intro,"
				+ "div.pb-container,"
				+ "div#echo-comments-overview,"
				+ "div.badgetext.hide.hidden"))
		{
			element.remove();
		}
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("https://www.washingtonpost.com/");
	}
}
