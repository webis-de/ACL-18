...
public class NationalReportWrapper extends Wrapper {
	@Override
	...
	private String getAuthor(Document jsoupDocument) {
		return jsoupDocument.select("div.author-description h3").text();
	}
	...
	private Document filter(Document jsoupDocument) {
		for(Element element : jsoupDocument.select("div#header-with-quote,"
				+ "p.wp-caption-text,"
				+ "div.comments-box,"
				+ "div#sidebar"))
		{
	...
	public boolean isValidUri(String targetUri) {
		return targetUri.contains("http://nationalreport.net/");
	}
}
